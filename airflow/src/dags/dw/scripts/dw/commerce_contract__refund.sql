...
SELECT
	...
	COALESCE(t2.refund_cc_id, t1.refund_cc_id) as refund_cc_id,
	COALESCE(t2.refund_cc_name, t1.refund_cc_name) as refund_cc_name,
	COALESCE(t2.refund_cr_id, t1.refund_cr_id) as refund_cr_id,
	COALESCE(t2.refund_cr_name, t1.refund_cr_name) as refund_cr_name,
...
FULL JOIN (
	...
	FROM (
		SELECT cr.id as refund_id
			...
			, NULL as refund_cc_id
			, NULL as refund_cc_name
			, NULL as refund_cr_id
			, NULL as refund_cr_name
		...
		WHERE cr.status = 1
			...
			AND (cr.remarks NOT RLIKE '合并|转|换|补|重|错' or cr.contract_id in(52339, 68826, 20826, 28589, 28606, 53078))
