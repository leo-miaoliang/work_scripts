...
insert overwrite table dw.teaching_lesson__student_class
-- 1.0 1v1
select
...
select
    ...
    ,case when t7.student_out_time is null or t7.student_out_time = 0 then null else from_unixtime(t7.student_out_time + 3600 * 8) end as student_out_time
    ,t6.teacher_evaluate as fb_to_teacher_score
    ,t6.evaluate_content as fb_to_teacher_comment
    ,case when t6.created is null or t6.created = 0 then null else from_unixtime(t6.created + 3600 * 8) end as fb_to_teacher_time
    ,current_timestamp as dw_load_time
