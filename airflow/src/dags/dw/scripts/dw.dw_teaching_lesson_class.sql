...
select
    ...
    ,case when t1.disabled = 1 and t1.cancel_type = 2 then 2  -- 2老师失约
    ...
    ,case when t10.last_cancel_time is null or t10.last_cancel_time = 0 then null else from_unixtime(t10.last_cancel_time + 3600 * 8, 'yyyy-MM-dd') end as cancel_time
...
select
    ...
    ,case when t10.last_cancel_time is null then null else from_unixtime(t10.last_cancel_time + 3600 * 8, 'yyyy-MM-dd') end as cancel_time
    ,from_unixtime(t1.start_time + 3600 * 8, 'yyyy-MM-dd') as class_date
    ,round((t1.end_time - t1.start_time) / 60, 0) as duration
    ...
    ,case when t8.created is null or t8.created = 0 then null else from_unixtime(t8.created + 3600 * 8) end as teacher_feedback_time
    ,case when t1.create_time is null or t1.create_time = 0 then null else from_unixtime(t1.create_time + 3600 * 8) end as create_time
    ,case when t1.update_time is null or t1.update_time = 0 then null else from_unixtime(t1.update_time + 3600 * 8) end as update_time
    ,current_timestamp as dw_load_date
from ods.newuuabc_class_appoint_course t1
left join
    (select
        t1.class_appoint_course_id
        ,sum(case when t1.disabled = 0 or (t1.disabled = 1 and t1.cancel_type = 2) then 1 else 0 end) as booked_student_cnt
...
select
    ...
    ,case when t1.attributes = 3 then 2  -- 2老师失约
        when from_unixtime(t1.start_time + 3600 * 8, 'yyyy-MM-dd') > '{{ macros.ds(ti) }}' then 1
        else 3
    end as class_status
    -- 1.0# 3学生请假4老师请假5系统障碍6其它 1.5# 4：学生改签
    -- -1.未知 1.学生请假 2.老师请假 3.系统障碍 4.其他
    ,case when t1.attributes = 3 or t1.disabled = 0 then null
        when t1.cancel_type = 1 then 4
        when t1.cancel_type = 3 then 1
        when t1.cancel_type = 4 then 2
        when t1.cancel_type = 5 then 3
        when t1.cancel_type = 6 then 4
        else -1
    end as cancel_type    -- 1学生失约2老师失约3学生请假4老师请假5系统障碍6其它
    ,null as cancel_time
    ,from_unixtime(t1.start_time + 3600 * 8, 'yyyy-MM-dd') as class_date
    ...
    ,t3.booked_student_cnt
    ,t3.booked_student_cnt as attended_student_cnt
    ...
    ,case when t1.create_time is null or t1.create_time = 0 then null else from_unixtime(t1.create_time + 3600 * 8) end as create_time
    ,case when t1.update_at is null or t1.update_at = 0 then null else from_unixtime(t1.update_at + 3600 * 8) end as update_time
    ,current_timestamp as dw_load_date
from ods.newuuabc_live_course t1
left join
    ods.newuuabc_teacher_user_new t2
...
select
    ...
    -- ,t2.courseware_id
    -- ,t3.courseware_name
    ,t1.clt_teacher_id as teacher_id
    ,t4.uid as teacher_sso_id
    ...
    ,case when t2.cl_course_type = 3 then 0 else t6.sum_sk + t6.sum_qj + t6.sum_kk + t6.sum_st end as booked_student_cnt
    ,case when t2.cl_course_type = 3 then 0 else t6.sum_sk end as attended_student_cnt
